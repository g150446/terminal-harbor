...
| `⌘1`〜`⌘9` | サイドバー左端の番号と同じ順の1〜9番目のワークスペースへ移動 |
| `⌘B` | サイドバーの表示・非表示を切り替え |
...
1行目の先頭はサイドバー順の1始まり番号で、`⌘1`〜`⌘9`と同じ並びです。番号のあとに
アクティビティ印と、現在のアクティブタブ内で選択されているペインのカレント
ディレクトリ名が続きます。フルパスではなく末尾名だけを表示します。2行目と3行目はAIエージェント
（`claude` / `codex` / `opencode` / Cursor CLI の `agent` と `cursor-agent`）の実行中のみ表示され、エージェント名と、
