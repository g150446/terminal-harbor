...
case $OSTYPE in
  darwin*)
    ...
    mkdir $zipdir
    cp -r 'assets/macos/Terminal Harbor.app' $zipdir/
    # Omit MetalANGLE for now; it's a bit laggy compared to CGL,
    # and on M1/Big Sur, CGL is implemented in terms of Metal anyway
    rm "$zipdir/Terminal Harbor.app"/*.dylib
    mkdir -p "$zipdir/Terminal Harbor.app/Contents/MacOS"
    mkdir -p "$zipdir/Terminal Harbor.app/Contents/Resources"
    cp -r assets/shell-integration/* "$zipdir/Terminal Harbor.app/Contents/Resources"
    cp -r assets/shell-completion "$zipdir/Terminal Harbor.app/Contents/Resources"
    tic -xe wezterm -o "$zipdir/Terminal Harbor.app/Contents/Resources/terminfo" termwiz/data/wezterm.terminfo
    ...
    for bin in wezterm wezterm-mux-server wezterm-gui strip-ansi-escapes ; do
      ...
      if [[ -f $TARGET_DIR/release/$bin ]] ; then
        cp $TARGET_DIR/release/$bin "$zipdir/Terminal Harbor.app/Contents/MacOS/$bin"
      else
        ...
        lipo $TARGET_DIR/*/release/$bin -output "$zipdir/Terminal Harbor.app/Contents/MacOS/$bin" -create
      fi
    done
    ...
    if [ -n "$MACOS_TEAM_ID" ] ; then
      ...
      /usr/bin/codesign --keychain build.keychain --force --options runtime \
        --entitlements ci/macos-entitlement.plist --deep --sign "$MACOS_TEAM_ID" "$zipdir/Terminal Harbor.app/"
